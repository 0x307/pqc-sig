...
/// - **This is a breaking change, shipped as one.** `multicodec_code()` previously returned
///   `Err(..)` for FN-DSA; `STABILITY.md` §2 lists "changing documented behavior... including
///   error-vs-success outcomes" as breaking, explicitly and without a doc-comment-worded-it-as-
///   provisional exception. It ships in a minor version bump alongside another, independently
///   breaking change in this same release — see `CHANGELOG.md`'s migration note.
