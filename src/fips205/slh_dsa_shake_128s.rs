...
use slh_dsa::{SigningKey, VerifyingKey, Shake128s};
...
/// SLH-DSA-SHAKE-128s keypair (NIST FIPS 205, Security Level 1, small signatures).
pub struct SlhDsaShake128sKeypair {
    signing_key: SigningKey<Shake128s>,
}

impl SlhDsaShake128sKeypair {
    ...
    pub fn generate<R: CryptoRng + RngCore>(rng: &mut R) -> SigResult<Self> {
        ...
        Ok(Self { signing_key: SigningKey::<Shake128s>::slh_keygen_internal(&sk_seed, &sk_prf, &pk_seed) })
    }
    pub fn public_key(&self) -> SigPublicKey {
        SigPublicKey::new(SigAlgorithm::SlhDsaShake128s, self.signing_key.verifying_key().to_bytes().to_vec())
    }
    pub fn secret_key(&self) -> SigSecretKey {
        SigSecretKey::new(SigAlgorithm::SlhDsaShake128s, self.signing_key.to_bytes().to_vec())
    }
    pub fn sign<R: CryptoRng + RngCore>(&self, rng: &mut R, message: &[u8]) -> SigResult<Signature> {
        ...
        Ok(Signature::new(SigAlgorithm::SlhDsaShake128s, sig.to_bytes().to_vec()))
    }
    ...
    pub fn verify(public_key: &SigPublicKey, message: &[u8], signature: &Signature) -> SigResult<()> {
        if public_key.algorithm != SigAlgorithm::SlhDsaShake128s {
            return Err(SigError::InvalidPublicKey(format!("expected SLH-DSA-SHAKE-128s key, got {:?}", public_key.algorithm)));
        }
        if signature.algorithm != SigAlgorithm::SlhDsaShake128s {
            return Err(SigError::InvalidSignature(format!("expected SLH-DSA-SHAKE-128s signature, got {:?}", signature.algorithm)));
        }
        let vk = VerifyingKey::<Shake128s>::try_from(public_key.bytes.as_slice())
            .map_err(|e| SigError::InvalidPublicKey(format!("{:?}", e)))?;
        let sig = slh_dsa::Signature::<Shake128s>::try_from(signature.bytes.as_slice())
    ...
    pub fn sign_prehash<R: CryptoRng + RngCore>(
        ...
        check_ctx(ctx)?;
        check_prehash(hash, digest, Self::SECURITY_STRENGTH_BITS, "SLH-DSA-SHAKE-128s")?;
    ...
    pub fn from_secret_key_bytes(bytes: &[u8]) -> SigResult<Self> {
        let signing_key = SigningKey::<Shake128s>::try_from(bytes)
