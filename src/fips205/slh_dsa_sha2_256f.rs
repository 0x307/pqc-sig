...
use slh_dsa::{SigningKey, VerifyingKey, Sha2_256f};
...
/// SLH-DSA-SHA2-256f keypair (NIST FIPS 205, Security Level 5, fast signing).
pub struct SlhDsaSha2_256fKeypair {
    signing_key: SigningKey<Sha2_256f>,
}

impl SlhDsaSha2_256fKeypair {
    ...
    pub fn generate<R: CryptoRng + RngCore>(rng: &mut R) -> SigResult<Self> {
        ...
        Ok(Self { signing_key: SigningKey::<Sha2_256f>::slh_keygen_internal(&sk_seed, &sk_prf, &pk_seed) })
    }
    pub fn public_key(&self) -> SigPublicKey {
        SigPublicKey::new(SigAlgorithm::SlhDsaSha2_256f, self.signing_key.verifying_key().to_bytes().to_vec())
    }
    pub fn secret_key(&self) -> SigSecretKey {
        SigSecretKey::new(SigAlgorithm::SlhDsaSha2_256f, self.signing_key.to_bytes().to_vec())
    }
    pub fn sign<R: CryptoRng + RngCore>(&self, rng: &mut R, message: &[u8]) -> SigResult<Signature> {
        ...
        Ok(Signature::new(SigAlgorithm::SlhDsaSha2_256f, sig.to_bytes().to_vec()))
    }
    ...
    pub fn verify(public_key: &SigPublicKey, message: &[u8], signature: &Signature) -> SigResult<()> {
        if public_key.algorithm != SigAlgorithm::SlhDsaSha2_256f {
            return Err(SigError::InvalidPublicKey(format!("expected SLH-DSA-SHA2-256f key, got {:?}", public_key.algorithm)));
        }
        if signature.algorithm != SigAlgorithm::SlhDsaSha2_256f {
            return Err(SigError::InvalidSignature(format!("expected SLH-DSA-SHA2-256f signature, got {:?}", signature.algorithm)));
        }
        let vk = VerifyingKey::<Sha2_256f>::try_from(public_key.bytes.as_slice())
            .map_err(|e| SigError::InvalidPublicKey(format!("{:?}", e)))?;
        let sig = slh_dsa::Signature::<Sha2_256f>::try_from(signature.bytes.as_slice())
    ...
    pub fn sign_prehash<R: CryptoRng + RngCore>(
        ...
        check_ctx(ctx)?;
        check_prehash(hash, digest, Self::SECURITY_STRENGTH_BITS, "SLH-DSA-SHA2-256f")?;
    ...
    pub fn from_secret_key_bytes(bytes: &[u8]) -> SigResult<Self> {
        let signing_key = SigningKey::<Sha2_256f>::try_from(bytes)
