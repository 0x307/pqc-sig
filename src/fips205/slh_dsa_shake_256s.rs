...
//!
//! Parameter sizes:
//! - Public key:    64 bytes  (N=32)
//! - Secret key:   128 bytes  (N=32)
//! - Signature:  29792 bytes

extern crate alloc;
use alloc::format;

use slh_dsa::{SigningKey, VerifyingKey, Shake256s};
...
/// SLH-DSA-SHAKE-256s keypair (NIST FIPS 205, Security Level 5, small signatures).
pub struct SlhDsaShake256sKeypair {
    signing_key: SigningKey<Shake256s>,
}

impl SlhDsaShake256sKeypair {
    ...
    pub fn generate<R: CryptoRng + RngCore>(rng: &mut R) -> SigResult<Self> {
        ...
        Ok(Self { signing_key: SigningKey::<Shake256s>::slh_keygen_internal(&sk_seed, &sk_prf, &pk_seed) })
    }
    pub fn public_key(&self) -> SigPublicKey {
        SigPublicKey::new(SigAlgorithm::SlhDsaShake256s, self.signing_key.verifying_key().to_bytes().to_vec())
    }
    pub fn secret_key(&self) -> SigSecretKey {
        SigSecretKey::new(SigAlgorithm::SlhDsaShake256s, self.signing_key.to_bytes().to_vec())
    }
    pub fn sign<R: CryptoRng + RngCore>(&self, rng: &mut R, message: &[u8]) -> SigResult<Signature> {
        // N=32 for SHAKE-256 parameter sets.
        let mut opt_rand = [0u8; 32];
        ...
        Ok(Signature::new(SigAlgorithm::SlhDsaShake256s, sig.to_bytes().to_vec()))
    }
    ...
    pub fn verify(public_key: &SigPublicKey, message: &[u8], signature: &Signature) -> SigResult<()> {
        if public_key.algorithm != SigAlgorithm::SlhDsaShake256s {
            return Err(SigError::InvalidPublicKey(format!("expected SLH-DSA-SHAKE-256s key, got {:?}", public_key.algorithm)));
        }
        if signature.algorithm != SigAlgorithm::SlhDsaShake256s {
            return Err(SigError::InvalidSignature(format!("expected SLH-DSA-SHAKE-256s signature, got {:?}", signature.algorithm)));
        }
        let vk = VerifyingKey::<Shake256s>::try_from(public_key.bytes.as_slice())
            .map_err(|e| SigError::InvalidPublicKey(format!("{:?}", e)))?;
        let sig = slh_dsa::Signature::<Shake256s>::try_from(signature.bytes.as_slice())
    ...
    pub fn sign_prehash<R: CryptoRng + RngCore>(
        ...
        check_ctx(ctx)?;
        check_prehash(hash, digest, Self::SECURITY_STRENGTH_BITS, "SLH-DSA-SHAKE-256s")?;
    ...
    pub fn from_secret_key_bytes(bytes: &[u8]) -> SigResult<Self> {
        let signing_key = SigningKey::<Shake256s>::try_from(bytes)
